...
for i in sub_folders:
    ...
    ##TO DO check weird values in console to make sure this is working. it shouldn't be giving values over 1?
    for (a, b) in zip(A_arr, B_arr):
        corr = signal.correlate(a, b)
        corr_norm = cc.normalize(corr)
    ...
    df_c.to_csv(f'{i}\\{new_folder}\\{os.path.basename(i).split("_")[0]}_{signal_A.split(".")[0]}_VS_{signal_B.split(".")[0]}.csv')
...
df_grp.to_csv(f"{path_to_folders}_NAc_VS_LHA_{day}")
